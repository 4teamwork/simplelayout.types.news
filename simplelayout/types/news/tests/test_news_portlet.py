...
class TestNewsPortlet(MockTestCase):
    ...
    def test_path(self):
        ...
        self.mock_tool(url, 'portal_url')
        self.expect(url.getPortalPath()).result('/plone')
        self.expect(context.getPhysicalPath()).result(('folder', 'file')).count(0, None)
        self.expect(catalog.searchResults(MATCH(query_matcher))).count(1).result(['1', '2', '3', '4', '5'])
        self.replay()
    ...
    def test_ClassificationItem(self):
        ...
        self.expect(url.getPortalPath()).result('/plone')
        self.expect(context.restrictedTraverse('/plone/hans/peter')).result(obj_uid)
        self.expect(context.getPhysicalPath()).result(('folder', 'file')).count(0, None)
        self.expect(catalog.searchResults(MATCH(query_matcher))).count(1).result(['1', '2', '3', '4', '5'])
        self.replay()
        obj = Assignment(classification_items=['hans/peter'], only_context=False)
        renderer = Renderer(context, request, view, manager, obj)
        renderer.get_news()
